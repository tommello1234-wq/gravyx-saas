...
export function CreateProjectModal({
  ...
  // Fetch ALL templates (no tier filter)
  const { data: templates, isLoading: templatesLoading } = useQuery({
    queryKey: ['project-templates-all'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('project_templates')
        .select('id, name, description, thumbnail_url, allowed_tiers')
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-1">
                    {templates.map((template) => {
                      ...
                      return (
                        <Card
                          key={template.id}
                          className={`cursor-pointer transition-all group overflow-hidden relative ${
                            locked
                              ? 'opacity-80 hover:border-muted-foreground/50'
                            ...
                            {template.thumbnail_url ? (
                              <img
                                ...
                                className={`w-full h-full object-cover transition-transform ${
                                  locked ? 'blur-[2px] brightness-75' : 'group-hover:scale-105'
                                }`}
                              />
                            ) : (
                              <div className={`w-full h-full flex items-center justify-center ${locked ? 'opacity-50' : ''}`}>
