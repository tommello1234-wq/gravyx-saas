...
interface SettingsNodeData { label: string; aspectRatio: string; quantity: number; }
...
export const SettingsNode = memo(({ data, id }: NodeProps) => {
  ...
  const handleAspectChange = useCallback((value: string) => { setAspectRatio(value); setNodes((nodes) => nodes.map((node) => node.id === id ? { ...node, data: { ...node.data, aspectRatio: value } } : node)); }, [id, setNodes]);
  const handleQuantityChange = useCallback((value: number) => { setQuantity(value); setNodes((nodes) => nodes.map((node) => node.id === id ? { ...node, data: { ...node.data, quantity: value } } : node)); }, [id, setNodes]);
  const handleReset = useCallback(() => { setAspectRatio('1:1'); setQuantity(1); setNodes(nodes => nodes.map(n => n.id === id ? { ...n, data: { ...n.data, aspectRatio: '1:1', quantity: 1 } } : n)); setEdges(edges => edges.filter(e => e.source !== id && e.target !== id)); }, [id, setNodes, setEdges]);
  ...
  const handleLabelKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter') setIsEditing(false); else if (e.key === 'Escape') { setLabel(nodeData.label || t('editor.settings')); setIsEditing(false); } };
  ...
  return <div className="bg-card border border-slate-500/30 rounded-2xl min-w-[300px] shadow-2xl shadow-slate-500/10">
    ...
    <div className="flex items-center justify-between p-4 border-b border-border/30 bg-gradient-to-r from-slate-500/10 to-transparent rounded-t-2xl">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-500 to-slate-600 flex items-center justify-center shadow-lg shadow-slate-500/30"><Settings className="h-5 w-5 text-white" /></div>
        <div>
          ...
          <p className="text-xs text-muted-foreground">{t('editor.configure_output')}</p>
        </div>
      </div>
      <DropdownMenu>
    ...
    <div className="p-4 space-y-5">
      <div className="space-y-3">
        <label className="text-sm font-medium text-muted-foreground">{t('editor.aspect_ratio')}</label>
        <div className="grid grid-cols-4 gap-2">
          {aspectRatios.map(ar => { const Icon = ar.icon; return <button key={ar.value} onClick={() => handleAspectChange(ar.value)} className={cn('flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all', aspectRatio === ar.value ? 'bg-primary/20 border-primary text-primary shadow-lg shadow-primary/20' : 'bg-muted/20 border-border/50 text-muted-foreground hover:border-border hover:bg-muted/40')}><Icon className={cn('h-5 w-5', ar.value === '16:9' && 'rotate-90')} /><span className="text-xs font-medium">{ar.label}</span></button>; })}
        </div>
      </div>
      <div className="space-y-3">
        <label className="text-sm font-medium text-muted-foreground">{t('editor.quantity_label')}</label>
        <div className="grid grid-cols-3 gap-2">
          {quantities.map(q => (<button key={q} onClick={() => handleQuantityChange(q)} className={cn('flex flex-col items-center gap-1 p-3 rounded-xl border transition-all', quantity === q ? 'bg-primary/20 border-primary text-primary shadow-lg shadow-primary/20' : 'bg-muted/20 border-border/50 text-muted-foreground hover:border-border hover:bg-muted/40')}><span className="text-lg font-bold">{q}</span><span className="text-xs">{q === 1 ? t('editor.image') : t('editor.images')}</span></button>))}
        </div>
      </div>
      <Button className="w-full h-12 rounded-xl bg-gradient-to-r from-violet-600 via-purple-600 to-violet-600 hover:from-violet-500 hover:via-purple-500 hover:to-violet-500 text-white font-semibold shadow-lg shadow-violet-500/30 transition-all hover:shadow-xl hover:shadow-violet-500/40 disabled:opacity-50 disabled:cursor-not-allowed" onClick={() => window.dispatchEvent(new CustomEvent(GENERATE_IMAGE_EVENT))} disabled={!hasEnoughCredits || isGenerating || jobQueueState.hasQueuedJobs || jobQueueState.hasProcessingJobs}>
        {jobQueueState.hasProcessingJobs ? (<><Loader2 className="h-5 w-5 mr-2 animate-spin" />{t('editor.generating')} {jobQueueState.totalPendingImages} {jobQueueState.totalPendingImages === 1 ? t('editor.image') : t('editor.images')}...</>) : jobQueueState.hasQueuedJobs ? (<><Loader2 className="h-5 w-5 mr-2 animate-spin" />{t('editor.in_queue')} ({jobQueueState.totalPendingImages})...</>) : isGenerating ? (<><Loader2 className="h-5 w-5 mr-2 animate-spin" />{t('editor.sending')}</>) : (<><Sparkles className="h-5 w-5 mr-2" />{t('editor.generate')} {quantity} {quantity === 1 ? t('editor.image') : t('editor.images')}</>)}
      </Button>
      <p className={cn("text-center text-xs", hasEnoughCredits ? "text-muted-foreground" : "text-destructive")}>
