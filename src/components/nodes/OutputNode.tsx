...
export const OutputNode = memo(({
  ...
  return <div className="bg-card/95 backdrop-blur-sm border border-border/50 rounded-2xl min-w-[320px] shadow-xl">
      <Handle type="target" position={Position.Left} className="!w-4 !h-4 !bg-violet-500 !border-4 !border-background !-left-2" />
      ...
      <div className="flex items-center justify-between p-4 border-b border-border/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-primary-foreground text-center">Resultado</h3>
            <p className="text-xs text-muted-foreground text-left">Salva automaticamente</p>
          </div>
        </div>
        ...
        {isLoading ? <div className="flex flex-col items-center justify-center py-12">
            ...
            <div className="grid grid-cols-2 gap-3">
              {images.map((url, index) => <div key={index} className="relative group">
                  <img src={url} alt={`Generated ${index + 1}`} className="w-full aspect-square object-cover rounded-xl" />
                  <Button variant="secondary" size="icon" className="absolute bottom-2 right-2 h-8 w-8 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 backdrop-blur-sm" onClick={() => downloadImage(url, index)}>
          ...
          </div> : <div className="border-2 border-dashed border-border/50 rounded-xl p-8 text-center">
