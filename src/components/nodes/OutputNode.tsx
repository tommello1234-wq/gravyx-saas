...
interface OutputNodeData { label: string; images: NodeImage[] | string[]; }
...
export const OutputNode = memo(({ data, id }: NodeProps) => {
  ...
  const handleReset = useCallback(() => { setNodes(nodes => nodes.map(n => n.id === id ? { ...n, data: { ...n.data, images: [] } } : n)); setEdges(edges => edges.filter(e => e.source !== id && e.target !== id)); }, [id, setNodes, setEdges]);
    ...
    <div className="bg-card border border-emerald-500/30 rounded-2xl min-w-[320px] shadow-2xl shadow-emerald-500/10">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border/30 bg-gradient-to-r from-emerald-500/10 to-transparent rounded-t-2xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/30"><Sparkles className="h-5 w-5 text-white" /></div>
          <div>
            ...
            <p className="text-xs text-muted-foreground">{images.length > 0 ? `${images.length} ${images.length > 1 ? t('editor.images') : t('editor.image')}` : t('editor.click_to_save')}</p>
          </div>
        </div>
        <DropdownMenu>
      ...
      <div className="p-4 nowheel nodrag" onWheel={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
        {images.length > 0 ? <div className="space-y-4">
            ...
            <ScrollArea className="h-[200px] nowheel nodrag" onWheel={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
              <div className="grid grid-cols-2 gap-2 pr-2">
                {images.map((image, index) => (
                  <div key={`${image.url}-${index}`} className="relative group rounded-xl overflow-hidden cursor-pointer border border-border/30 hover:border-emerald-500/50 transition-all" onClick={() => handleImageClick(image)}>
                    <img src={image.url} alt={`Generated ${index + 1}`} className="w-full h-24 object-cover" />
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><span className="text-xs text-white font-medium">{t('editor.click_to_view')}</span></div>
                  </div>
                ))}
              </div>
            </ScrollArea>
          ...
          {images.length > 1 && <Button variant="outline" className="w-full rounded-xl border-border/50 bg-card text-foreground hover:bg-muted" onClick={downloadAll}><Download className="h-4 w-4 mr-2" />{t('editor.download_all')} ({images.length})</Button>}
        </div> : <div className="border-2 border-dashed border-border/30 rounded-xl p-8 text-center bg-muted/10">
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 flex items-center justify-center mx-auto mb-4"><Sparkles className="h-8 w-8 text-emerald-500/50" /></div>
          <p className="text-sm text-muted-foreground">{t('editor.images_will_appear')}</p>
        </div>}
      </div>
    </div>
