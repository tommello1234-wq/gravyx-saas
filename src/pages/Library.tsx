...
export default function Library() {
  ...
  const { data: references, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: images, error: imgError } = await supabase.from('reference_images').select('*').order('created_at', { ascending: false });
      ...
      if (submitterIds.length > 0) {
        const { data: profiles } = await supabase.from('profiles').select('user_id, display_name, avatar_url').in('user_id', submitterIds);
        profileMap = new Map((profiles || []).map(p => [p.user_id, { name: p.display_name, avatar_url: p.avatar_url }]));
      }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-8">
            ...
            <Button
              ...
              className={`rounded-full ${
                cat.value === 'all' && selectedCategory === cat.value
                  ...
                  : cat.value === 'all' ? 'border-primary/50' : ''
              }`}
        ...
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-12 text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">{t('library.no_reference')}</h3>
            <p className="text-muted-foreground">
              {selectedCategory === 'all' ? t('library.no_images_available') : t('library.no_images_category')}
            </p>
          </motion.div>
        ) : (
          <>
            <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
              {filteredImages.map((ref, index) => {
                ...
                return (
                  <motion.div key={ref.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index, 12) * 0.03 }} className="break-inside-avoid">
                    <div
                    ...
                    >
                      <img src={ref.image_url} alt={ref.title} className={`w-full object-cover transition-all ${isLocked ? 'blur-md scale-105' : ''}`} loading="lazy" />
                      {isLocked ? (
                        <div className="absolute inset-0 bg-background/60 flex flex-col items-center justify-center gap-2">
                          <Lock className="h-6 w-6 text-muted-foreground" />
                          <p className="text-sm text-muted-foreground font-medium">{t('library.paid_plan')}</p>
                        </div>
                      ) : (
                        <>
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {ref.tags.filter(tg => tg !== 'free').map(tag => (
                                <span key={tag} className="text-white/70 text-sm capitalize">{getCategoryLabel(tag)}</span>
                              ))}
                            </div>
                          </div>
                        </>
            ...
            {isFree && filteredImages.some(img => !img.tags.includes('free')) && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 p-6 rounded-xl border border-primary/30 bg-primary/5 text-center">
                <Lock className="h-8 w-8 text-primary mx-auto mb-3" />
                <h3 className="text-lg font-semibold mb-1">{t('library.locked_images')}</h3>
                <p className="text-muted-foreground text-sm mb-4">{t('library.upgrade_access')}</p>
                <Button onClick={() => setShowUpgrade(true)}>{t('home.see_plans')}</Button>
              </motion.div>
            )}
          </>
        )}
      ...
      <ImageViewerModal
        ...
        image={selectedImage ? {
          ...
          category: selectedImage.tags.filter(tg => tg !== 'free').map(getCategoryLabel).join(', '),
