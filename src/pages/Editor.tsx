...
export default function Editor() {
  ...
  useEffect(() => {
    const loadProject = async () => {
      ...
      if (error) {
        ...
        const canvasState = data.canvas_state as {
          nodes?: Node[];
          edges?: Edge[];
        };
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      const {
        ...
        canvas_state: JSON.parse(JSON.stringify({
          nodes,
          edges
        }))
      }).eq('id', projectId);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const promptNodes = inputEdges.map(e => nodes.find(n => n.id === e.source && n.type === 'prompt')).filter(Boolean) as Node[];
    const mediaNodes = inputEdges.map(e => nodes.find(n => n.id === e.source && n.type === 'media')).filter(Boolean) as Node[];
    ...
    const prompt = promptNodes.map(n => (n.data as {
      value: string;
    }).value).join(' ');
    const aspectRatio = (settingsNode.data as {
      aspectRatio: string;
    }).aspectRatio;

    // Get ALL image URLs from connected media nodes
    const imageUrls = mediaNodes.map(n => (n.data as {
      url: string | null;
    }).url).filter(Boolean) as string[];

    // Set loading state on output node
    setNodes(nds => nds.map(n => n.id === outputNode.id ? {
      ...n,
      data: {
        ...n.data,
        isLoading: true
  ...
  return <div className="h-screen flex flex-col bg-background">
      ...
      <div className="border-b border-border bg-card/50 backdrop-blur-sm px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {isSaving && <span className="text-xs text-muted-foreground flex items-center gap-1">
